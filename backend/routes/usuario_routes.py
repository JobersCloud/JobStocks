...
@usuario_bp.route('/<int:user_id>/visible-pedidos', methods=['PUT'])
@login_required
@csrf_required
@administrador_required
def toggle_visible_pedidos(user_id):
    """
    Activar/desactivar visibilidad de pedidos para un usuario (solo administradores)
    ---
    tags:
      - Usuarios
    security:
    ...
    """
    data = request.json
    if not data or 'visible_pedidos' not in data:
        return jsonify({
            'success': False,
            'error': 'Campo "visible_pedidos" es requerido'
        }), 400
    ...
    try:
        ...
        cursor.execute("""
            UPDATE users_empresas SET visible_pedidos = ?
            WHERE user_id = ? AND empresa_id = ?
        """, (1 if data['visible_pedidos'] else 0, user_id, empresa_id))
        ...
        estado = 'habilitados' if data['visible_pedidos'] else 'deshabilitados'
        return jsonify({
            'success': True,
            'message': f'Pedidos {estado} para el usuario'
        }), 200
    except Exception as e:
        return jsonify({
            'success': False,
