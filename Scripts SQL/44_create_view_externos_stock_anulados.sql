-- ============================================
-- Vista: view_externos_stock_anulados
-- Descripcion: Stock de productos ANULADOS con existencias disponibles
-- Base de datos: ApiRestStocks (o la BD de la instalacion)
-- Dependencia: Base de datos cristal (tablas externas)
-- NOTA: Esta vista es un TEMPLATE. Cada instalacion puede
--       necesitar ajustar nombres de tablas/campos segun su ERP.
-- ============================================
...
CREATE VIEW [dbo].[view_externos_stock_anulados]
AS
SELECT
...
WHERE
    articulos.fecha_anulado IS NOT NULL
    AND articulos.fecha_anulado <= GETDATE()
...
PRINT 'Vista view_externos_stock_anulados creada exitosamente';
